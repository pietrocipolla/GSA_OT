function [results]=bigwassersteintest(x,y,M,selection,entropy)
%% BIGWASSERSTEINTEST Optimal Transport for Sensitivity Wrapper
% BIGWASSERSTEINTEST(X,Y,M,MODES) returns a list of separations and
% computing time for each mode listed (either bit-pattern or list of
% strings) given the NxD matrix X as input and the NxL matrix Y as output.
% M denotes the number of partitions in X
...
if(nargin<5 || isempty(entropy))
    entropy=0; % default entropic parameter
end
modenames={'simplex','sinkhorn','swap','hungarian','habr','lawler','bures',...
    'gradient','divergence','sinkscale','greenkhorn','revsimplex','otpython','sinkmem','ldn'};
needcost=[1 1 0 1 1 1 0 0 1 1 1 1 1 0 1];
MAXBIT=length(modenames); % row*col>= MAXBIT fopr gfx

if(isstring(selection)||ischar(selection))
 modes=0;
 bit=find(strcmp(selection,modenames));
 if ~isempty(bit)
...
for m=1:M
    m
     for i=1:k
      %   i
         ii=ix(ms(m)+1:ms(m+1),i);nn=size(ii,1);
        [S,R]=sinkfastCost(entropy,C(ii,:)); % 400 for environ, 2 for normal
...
if bitget(selection,8)
    ...
    disp('Gradient');
    if(entropy==0)
        entropy=-0.001; % relative precision
        fprintf('Entropic penalty %g\n', entropy)
    end
    tic;
    W=wassgrad2(x,C,M,entropy); 
 t=toc
 results.(['W' type])=W;
 results.(['T' type])=t;
    ...
    m
     for i=1:k
      %   i
         ...
         [w,opts]=sinkhorn4([],entropy,y,y(ii,:),[],[],FF);
...
%     jj=round(linspace(.5,mc+.499,n)); % upscaling indices
    ...
    % i
     ...
     jj=randperm(n,nn);
     % downscale and compute
     a=sinkfastCost(entropy,C(ii,jj));
     W(m,i)=a/nn;
    end
end
t=toc
...
for m=1:M
    ...
    % i
     ...
     jj=randperm(n,nn);
     % downscale and compute
     a=greenkhorn(exp(-C(ii,jj)/2),ones(nn,1),ones(1,nn),entropy,false,...
         [],[]); % Greedy Sinkhorn
...
results.(['T' type])=t;
nextplot(cnt);cnt=cnt+1;
plot(1:M,W);title('Greenkhorn*');drawnow
end
...
if bitget(selection,15) % 0
 type=modenames{15};
 disp('linear diagonal network');
 W=zeros(M,k);
 if(entropy==0)
        entropy=0.05; 
        fprintf('Entropic penalty %g\n', entropy)
 ...
 for m=1:M
    m
     ...
     %    i
         ii=ix(ms(m)+1:ms(m+1),i);nn=size(ii,1);
         X=transportLDN(C(:,ii),ones(n,1)/n,ones(nn,1)/nn,entropy);
         W(m,i)=sum(X.*C(:,ii),'all');
     end
 end
 t=toc
 results.(['W' type])=W;
 results.(['T' type])=t;
 nextplot(cnt);cnt=cnt+1;
 plot(1:M,W);title('Linear Diagonal Network');drawnow
end
end % function
function testbig
%%
