...
for i=1:k
 ...
 if(nargin==4)
    if(k>1) 
	subplot(ceil(k/opts.PlotCols)+1,opts.PlotCols,i);
    else
        subplot(ceil(k/opts.PlotCols)+1,1,i); 
    end
    if(~opts.DDD)
...
%%% contribution to the variance of the conditional expectation
 ...
 %%%
    ...
    if(opts.PowerLoss~=0)
        %Sr(m)=opts.PowerLossScale*(mean(interp1(yy,f2,ys).^gamma)-EfYgm1);
...
if(~isempty(gfx))
   ...
   subplot(ceil(k/opts.PlotCols)+1,1,ceil(k/opts.PlotCols)+1);
   hold on
