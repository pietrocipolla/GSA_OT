...
for m=1:M
 ...
 if(l>1)     
  ...
  % conditional output
   ...
   Rc=chol(Cc);
   traceTerms0 = (sum(sum((Ry-Rc).^2))); % Frobenius norm on Cholesky root
   Ms(m,i)= meanSqDists;           
   W(m,i)= meanSqDists + traceTerms0;
      
   if(sigma~=0)
    D = sqrtm( 4*Ry*Cc*Ry + sigma^4*eye(size(Cy)));
