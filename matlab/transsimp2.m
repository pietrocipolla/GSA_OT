...
case 'NW2'
 ...
 root=1;
    case 'VOGEL',
        ...
        % Compute a penalty equal to the difference between the two
        % smallest costs (per row/col). Find the largest penalty, make this
        % a basic variable, make it as large as possible, cross out row or
        % column, change supply or demand. Repeat until only one uncrossed
        % cell remains and set the variable to the supply or demand
...
% alle ui und v j bestimmen.
  ...
  lowerval=2;upperval=2;
  for l=leavs
   k=l;
   while ~visited(k)
    visited(k)=true;
    wayBack(upperval)=k;
    upperval=upperval+1;k=abs(tree(k));
   end
   ...
   if(lowerval+1<upperval)
    ii(lowerval:upperval-1)=upperval-1:-1:lowerval; % revert
   end
   lowerval=upperval;
  end
