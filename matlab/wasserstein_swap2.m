function [sigma]=wasserstein_swap2(X,Y,precision)
...
% no change? optimum
 if ~updated
     if verbose, disp('Optimum found'); end
     converged=true;
 else
% accuracy reached ?
%  for i=1:n
...
%  end
 ...
 s=mean(abs(d)); 
  if(s<precision) 
      if verbose,disp('Precision reached'); end
      converged=true; 
  end
 end
